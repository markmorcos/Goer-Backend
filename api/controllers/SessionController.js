...
exports.signIn = function(req, res) {
    ...
    User.findOne({ email: req.body.email })
        ...
        .exec(function(err, user) {
            ...
            if (!user.approved)
                return res.status(401).json({ success: false, message: 'Account not approved yet' })
            ...
            bcrypt.compare(req.body.password, user.password, function(err, match) {
                ...
                Session.findOne({ user: user._id, registrationToken: req.body.registrationToken }, function(
                    err,
                    session
                ) {
                    if (err) return res.send(err)
...
exports.facebookSignIn = function(req, res) {
    ...
    axios
        .get(
            `https://graph.facebook.com/me?fields=id,first_name,last_name,email,picture{url},gender,locale,birthday,friends.limit(5000){id}&access_token=${
            ...
            User.findOneOrCreate(
                { email: profile.email },
                {
                    ...
                    birthdate: profile.birthday
                        ? moment(new Date(profile.birthday)).format('YYYY-MM-DD')
                        : undefined,
                ...
                function(err, user) {
                    ...
                    user.picture =
                        !user.picture || user.picture.indexOf('fbcdn.net') !== -1
                            ? profile.picture.data.url
                            : user.picture
                    user.save(function(err, user) {
                        var token = jwt.sign({ _id: user._id }, process.env.JWT_SECRET)
                        const facebookIds = profile.friends.data.map(function(friend) {
                            return friend.id
                        })
                        User.find({ facebook: { $in: facebookIds } }, 'name picture', function(err, users) {
                            if (err) return res.send(err)
                            Session.findOne(
                                { user: user._id, registrationToken: req.body.registrationToken },
                                function(err, session) {
                                    ...
                                    session.save(function(err, session) {
                                        ...
                                        res.json({
                                            success: true,
                                            data: { user: user, friends: users, token: token }
                                        })
                                    })
                                }
                            )
