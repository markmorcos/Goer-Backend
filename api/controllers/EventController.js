...
exports.list = function(req, res) {
    ...
    Thread.findById(req.query.thread, function(err, thread) {
        ...
        if (thread.users.indexOf(req.decoded._id) === -1) {
            return res
                .status(403)
                .json({ success: false, message: 'You are not allowed to read these events' })
        }
        Event.find({ thread: req.query.thread })
...
exports.create = function(req, res) {
    ...
    if (!req.body.startsAt)
        return res.status(400).json({ success: false, message: 'Start timestamp is required' })
    ...
    if (!req.body.endsAt)
        return res.status(400).json({ success: false, message: 'End timestamp is required' })
...
exports.rsvp = function(req, res) {
    ...
    if (req.body.rsvp === undefined)
        return res.status(400).json({ success: false, message: 'RSVP is required' })
