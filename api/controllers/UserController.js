...
exports.validateNewUser = function(req, res, next) {
    var valid = true
    if (!req.body.email)
        (valid = false), res.status(400).json({ success: false, message: 'Email is required' })
    User.findOne({ email: req.body.email }, function(err, user) {
        if (valid && err) (valid = false), res.send(err)
        if (valid && user)
            (valid = false), res.status(400).json({ success: false, message: 'Email is already taken' })
...
function populateUser(req, res, next) {
    var user = new User({
        ...
        location:
            req.body.latitude && req.body.longitude ? [req.body.latitude, req.body.longitude] : undefined,
...
exports.search = function(req, res) {
    ...
    switch (req.query.type) {
        ...
        case 'tags':
            ...
            query.role = 'business'
            query.tags = {
                $in: req.query.tags.map(function(tag) {
        ...
        case 'nearby':
            ...
            aggregate.unshift({
                $geoNear: {
                    near: {
                        type: 'Point',
                        coordinates: [Number(req.query.latitude), Number(req.query.longitude)]
                    },
        ...
        default:
            return res
                .status(400)
                .json({ success: false, message: 'Type should be either people, tags or nearby' })
    }
...
exports.validateExistingUser = function(req, res, next) {
    ...
    User.findById(req.body.id || req.decoded._id, function(err, user) {
        if (valid && err) (valid = false), res.send(err)
        if (valid && !user)
            (valid = false), res.status(404).json({ success: false, message: 'User not found' })
        if (valid && req.decoded._id != user._id) {
            ;(valid = false),
                res.status(403).json({ success: false, message: 'You are not allowed to update this user' })
        }
...
exports.list = function(req, res) {
    ...
    User.find({ role: 'user' }, { password: false })
        .sort('fullName')
